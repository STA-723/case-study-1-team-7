...
namesX0 = colnames(X0)
X0 = X0[,-c(which(namesX0=="EE0.01:labBayer"),
           which(namesX0=="EE0.03:labBayer"),
           which(namesX0=="EE0.01:labZeneca"))]
X1 = dplyr::select(data.frame(X0),starts_with("lab"))
X2 = dplyr::select(data.frame(X0),-starts_with("lab"))[,-1]
...
model = function(){
  ...
  # beta for others
  beta[(p_lab+1):p] ~ dmnorm(rep(0,p-p_lab), phi*tau*SSX2)
}
set.seed(1)
Xs = scale(X)
data = list(Y=Y, Xs=Xs, SSX2 = t(Xs[,(ncol(X1)+1):ncol(X)])%*%Xs[,(ncol(X1)+1):ncol(X)],
            n=length(Y), p_lab=ncol(X1), p=ncol(X), a=9)
