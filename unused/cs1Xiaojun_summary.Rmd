...
sensi_logistic<- data.frame(dde_glm = c(llm_coefs[2], llm_confint[2,]), pc1_glm =  c(llm_coefs[3], llm_confint[3,]), dde_infl = c(llm_coefs_infl[2], llm_confint_infl[2,]), pc1_infl =  c(llm_coefs_infl[3], llm_confint_infl[3,]))

ggplot(as.data.frame(sensi_logistic), aes(c("DDE", "PC1"), 
  fit, size=10, group=1, ylim=max(upr)+0.8)) +
  theme_bw(base_size=10)+ 
  geom_point(size=2)+
  geom_line(size=1)+
